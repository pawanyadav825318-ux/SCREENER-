...
from datasource import IST, OHLCV, apply_overlay, fetch_ohlc, fetch_quotes, fy_client, throttle
from tech import adx_dmi, atr, resample_ohlc, rsi, stochastic, vwap
...
def operand_ui(key, side, default_name):
    ...
    return {"ind": name, "params": params, "field": field, "off": off}
...
def clause_ui(prefix, cid, n):
    ...
    with st.container(border=True):
        ...
        if head[1].button("❌", key=f"{prefix}del{cid}"):
            deleted = True
...
def group_ui(gid, n, allow_delete):
    ...
    with st.container(border=True):
        ...
        if allow_delete and top[1].button("❌ Group hatao", key=f"gdel{gid}"):
            del_group = True
        ...
        for i, cid in enumerate(list(ss[f"g{gid}_ids"]), 1):
            ...
            if deleted and len(ss[f"g{gid}_ids"]) > 1:
                ...
                st.rerun()
            clauses.append(c)
        if st.button("➕ Condition jodo (indicator add karo)", key=f"g{gid}addc"):
...
# ---------------- Save/Load scan (session state <-> plain dict) ----------------
def collect_config():
    """Abhi UI mein bane groups/join ko ek plain (JSON-safe) dict mein utaarta hai, save karne ke liye."""
    ss = st.session_state
    grp_ids = list(ss.get("grp_ids", [0]))
    cfg = {"grp_ids": grp_ids, "grp_next": ss.get("grp_next", 1),
           "grp_join": ss.get("grp_join", "OR (koi ek group sach ho)"), "groups": {}}
    for gid in grp_ids:
        cids = list(ss.get(f"g{gid}_ids", []))
        gdict = {"ids": cids, "next": ss.get(f"g{gid}_next", 1),
                 "join": ss.get(f"g{gid}join", "AND (sabhi sach ho)"), "clauses": {}}
        for cid in cids:
            prefix = f"g{gid}_c{cid}"
            gdict["clauses"][str(cid)] = {
                "tf": ss.get(f"g{gid}_tf{cid}", "Chart timeframe"),
                "op": ss.get(f"g{gid}_op{cid}", ">"),
                "within": ss.get(f"g{gid}_w{cid}", 1),
                "L": _collect_operand(f"{prefix}L"),
                "R": _collect_operand(f"{prefix}R"),
            }
        cfg["groups"][str(gid)] = gdict
    return cfg


def _collect_operand(key):
    ss = st.session_state
    name = ss.get(f"{key}_n")
    if name is None:
        return None
    out = {"n": name}
    if name == "Number":
        out["v"] = ss.get(f"{key}_v", 30.0)
        return out
    plist = IND.get(name, ([], None, None))[0]
    out["p"] = {pid: ss.get(f"{key}_p_{pid}") for pid, _, _ in plist}
    out["f"] = ss.get(f"{key}_f")
    out["o"] = ss.get(f"{key}_o", "Latest")
    return out


def apply_config(cfg):
    """Saved config ko wapas session_state mein daal deta hai (agli render par UI usi se bharegi)."""
    ss = st.session_state
    for k in list(ss.keys()):
        if k.startswith("g") and ("_ids" in k or "_next" in k or "join" in k or "_n" in k or "_v" in k
                                   or "_p_" in k or "_f" in k or "_o" in k or "op" in k or "_w" in k
                                   or "_tf" in k or k.startswith("gdel") or k.startswith("gaddc")):
            del ss[k]
    ss["grp_ids"] = list(cfg["grp_ids"])
    ss["grp_next"] = cfg["grp_next"]
    ss["grp_join"] = cfg["grp_join"]
    for gid_s, g in cfg["groups"].items():
        gid = int(gid_s)
        ss[f"g{gid}_ids"] = list(g["ids"])
        ss[f"g{gid}_next"] = g["next"]
        ss[f"g{gid}join"] = g["join"]
        for cid_s, c in g["clauses"].items():
            cid = int(cid_s)
            ss[f"g{gid}_tf{cid}"] = c["tf"]
            ss[f"g{gid}_op{cid}"] = c["op"]
            ss[f"g{gid}_w{cid}"] = c["within"]
            _apply_operand(f"g{gid}_c{cid}L", c["L"])
            _apply_operand(f"g{gid}_c{cid}R", c["R"])


def _apply_operand(key, o):
    if not o:
        return
    ss = st.session_state
    ss[f"{key}_n"] = o["n"]
    if o["n"] == "Number":
        ss[f"{key}_v"] = o.get("v", 30.0)
        return
    for pid, v in o.get("p", {}).items():
        ss[f"{key}_p_{pid}"] = v
    if o.get("f") is not None:
        ss[f"{key}_f"] = o["f"]
    ss[f"{key}_o"] = o.get("o", "Latest")
...
def _val(sym, o, back, cache, chart_frames, daily_frames, chart_intraday, tf):
    if "num" in o:
        return o["num"]
            ...
            for g in groups:
                for c in g["clauses"]:
                    ...
                    for o in (c["left"], c["right"]):
                        if "ind" in o:
                            label = op_text(o) + ("" if tf == "Chart timeframe" else f" [{tf}]")
                            row[label] = _val(sym, o, 0, cache, chart_frames, daily_frames, chart_intraday, tf)
            rows[sym] = row
        except Exception:
            continue
