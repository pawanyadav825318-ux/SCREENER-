"""
Pawan Technical Screener (Chartink-style) - NSE, Fyers real-time
UI file. Data: datasource.py | Indicators: tech.py | Builder: builder.py
Save/Load: scans_store.py | Alerts: notify.py
"""
import time
...
from datasource import (HIST_TTL, IST, TFS, apply_overlay, build, fetch_ohlc, fetch_quotes,
                        fyers_configured, fyers_session, fyers_token, market_is_open, secret, token_store)
from builder import apply_config, builder_ui, collect_config, describe, load_daily_long, load_frames, run_scan
from scans_store import delete_scan, export_all, import_all, list_scans, load_scan, save_scan
from notify import notify_panel
from tech import indicators

st.set_page_config(page_title="Pawan Technical Screener", page_icon="📈", layout="wide")

CSS = """
<style>
#MainMenu, footer {visibility: hidden;}
h1 {font-weight: 800 !important; letter-spacing: -0.5px;}
div[data-testid="stMetric"] {background: #1b2028; border: 1px solid #2a3140; border-radius: 10px; padding: 10px;}
div[data-testid="stDataFrame"] {border: 1px solid #2a3140; border-radius: 10px;}
.stTabs [data-baseweb="tab"] {font-weight: 600;}
div[data-testid="stExpander"] {border: 1px solid #2a3140; border-radius: 10px;}
button[kind="secondary"] {border-radius: 8px !important;}
</style>
"""
st.markdown(CSS, unsafe_allow_html=True)

TIMEFRAME_LABELS = {"1m": "1 min", "3m": "3 min", "5m": "5 min", "15m": "15 min", "30m": "30 min",
                    "1h": "1 ghanta", "2h": "2 ghante", "3h": "3 ghante", "4h": "4 ghante",
                    "1d": "Daily (D)", "1w": "Weekly (W)", "1M": "Monthly (M)"}
...
NIFTY_NEXT50 = """ADANIENSOL ADANIGREEN ADANIPOWER AMBUJACEM BAJAJHFL BAJAJHLDNG BANKBARODA BOSCHLTD CANBK CGPOWER
CHOLAFIN DABUR DLF DMART GAIL GODREJCP HAL HAVELLS HDFCAMC ICICIGI ICICIPRULI INDHOTEL INDIGO IOC IRFC
JINDALSTEL JSWENERGY LODHA LTIM MOTHERSON NHPC PFC PIDILITIND PNB RECLTD SIEMENS SWIGGY TATAPOWER TIINDIA
TORNTPHARM TVSMOTOR UNITDSPR UNIONBANK VBL VEDL ZOMATO ZYDUSLIFE""".split()

NIFTY_MID_EXTRA = """AUBANK ALKEM APLAPOLLO ASHOKLEY ASTRAL AUROPHARMA BALKRISHNA BANDHANBNK BANKINDIA BATAINDIA
BERGEPAINT BHARATFORG BHEL BIOCON COFORGE COLPAL CONCOR COROMANDEL CUMMINSIND DEEPAKNTR DIXON ESCORTS
EXIDEIND FEDERALBNK FORTIS GLAND GMRAIRPORT GODREJPROP HDFCLIFE HONAUT IDEA IDFCFIRSTB IGL INDIANB
INDUSTOWER IPCALAB JUBLFOOD KPITTECH L&TFH LICHSGFIN LUPIN MANKIND MARICO MAXHEALTH MFSL MPHASIS
MRF NATIONALUM NAUKRI NMDC OBEROIRLTY OFSS OIL PAGEIND PATANJALI PERSISTENT PETRONET PHOENIXLTD PIIND
POLYCAB PRESTIGE SAIL SBICARD SOLARINDS SONACOMS SRF STARHEALTH SUNDARMFIN SUPREMEIND SUZLON TATACOMM
TATAELXSI TATATECH TORNTPOWER TRENT TUBEINVEST UBL UPL VOLTAS YESBANK""".split()

FNO_EXTRA = """ABCAPITAL ABFRL APOLLOTYRE ASTRAZEN ATUL BALRAMCHIN BANDHANBNK BATAINDIA BHARATFORG BSOFT CANFINHOME
CENTURYTEX CHAMBLFERT CHOLAHLDNG CROMPTON CUB DALBHARAT DELHIVERY GLENMARK GNFC GRANULES GUJGASLTD HFCL
IBULHSGFIN IDFC IEX INDIACEM INDUSTOWER IRB JKCEMENT LAURUSLABS LTTS M&MFIN MANAPPURAM MCX METROPOLIS
NATCOPHARM NAVINFLUOR OBEROIRLTY PEL PFIZER PVRINOX RAIN RAMCOCEM RBLBANK SAIL SUNTV SYNGENE TATACHEM
TATACOMM TVSMOTOR UNOMINDA""".split()

UNIVERSES = {
    "Nifty 50": NIFTY50,
    "Nifty 100": NIFTY50 + NIFTY_NEXT50,
    "Nifty 200": NIFTY50 + NIFTY_NEXT50 + NIFTY_MID_EXTRA,
    "F&O Stocks": NIFTY50 + NIFTY_NEXT50 + NIFTY_MID_EXTRA + FNO_EXTRA,
    "Custom": None,
}
...
def make_chart(o, symbol, intraday):
    ...
    fig.update_layout(height=760, margin=dict(l=5, r=5, t=30, b=5), legend=dict(orientation="h", y=1.04),
                      title=f"{symbol} • {len(o)} candles", template="plotly_dark")
    return fig


def save_load_panel():
    with st.expander("💾 Save / Load Scan"):
        c1, c2 = st.columns([3, 1])
        nm = c1.text_input("Scan ka naam", key="save_name", label_visibility="collapsed",
                           placeholder="Naam do, jaise 'RSI reversal'")
        if c2.button("💾 Save", width="stretch") and nm.strip():
            ok = save_scan(nm.strip(), collect_config())
            st.success("Saved ✅") if ok else st.error("Save nahi hua")
        scans = list_scans()
        if scans:
            c3, c4, c5 = st.columns([3, 1, 1])
            pick = c3.selectbox("Saved scans", scans, label_visibility="collapsed")
            if c4.button("📂 Load", width="stretch"):
                cfg = load_scan(pick)
                if cfg:
                    apply_config(cfg)
                    st.rerun()
            if c5.button("🗑️ Delete", width="stretch"):
                delete_scan(pick)
                st.rerun()
        else:
            st.caption("Abhi koi saved scan nahi hai.")
        st.caption("⚠️ Yeh app ke chalte rehne tak save rehta hai. App reboot/redeploy hone par mit sakta hai — "
                  "isliye neeche se backup zaroor download kar lo.")
        bc1, bc2 = st.columns(2)
        bc1.download_button("⬇️ Sab scans ka backup (.json)", export_all(), "scans_backup.json", "application/json",
                            width="stretch")
        up = bc2.file_uploader("⬆️ Backup se restore karo", type="json", label_visibility="collapsed")
        if up is not None:
            ok, n = import_all(up.read().decode())
            st.success(f"{n} scans restore ho gaye ✅") if ok else st.error("Restore fail")
...
if pw and not st.session_state.get("authed"):
    st.title("🔒 Pawan Technical Screener")
...
with st.sidebar:
    ...
    uni_name = st.selectbox("Symbol list", list(UNIVERSES))
    if uni_name == "Custom":
        ...
        symbols = tuple(dict.fromkeys(UNIVERSES[uni_name]))
        st.caption(f"{len(symbols)} symbols")
    tf = st.selectbox("Timeframe", TFS, index=3, format_func=lambda k: TIMEFRAME_LABELS[k])
...
now_ts = time.time()
hb = int(now_ts // HIST_TTL.get(tf, 300))
qb = int(now_ts // 10)
...
st.caption(f"{len(data)} stocks • {TIMEFRAME_LABELS[tf]} • {tag} • updated {datetime.now(IST):%H:%M:%S}"
...
with tab_scan:
    ...
    if mode == "🧱 Chartink builder":
        ...
        table = run_scan(chart_frames, daily_frames, groups, top_join, tf not in ("1d", "1w", "1M"))
    else:
        if mode == "Ready scans":
...
with tab_chart:
    ...
    try:
        frames, _, _ = fetch_ohlc(symbols, tf, source, token, hb)
        q = fetch_quotes(tuple(frames), token, qb) if (source == "fyers" and live) else {}
        df = apply_overlay(frames[pick], q.get(pick), tf)
        st.plotly_chart(make_chart(indicators(df, tf not in ("1d", "1w", "1M")), pick,
                                   tf not in ("1d", "1w", "1M")), width="stretch")
    except Exception as e:
